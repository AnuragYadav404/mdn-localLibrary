...
// userArgs contains mongoDB url

// import the "models" from their modules
const Book = require("./models/book");
const Author = require("./models/author");
const Genre = require("./models/genre");
const BookInstance = require("./models/bookinstance");

// create an array for each instances
const genres = [];
const authors = [];
const books = [];
const bookinstances = [];
...
async function main() {
  ...
  // connected else error is catch in catch stmt
  // no ref used by genre
  await createGenres();
  // no ref used by authors
  await createAuthors();
...
async function genreCreate(index, name) {
  // instantiate a new genre
  const genre = new Genre({ name: name });
  // save the genre
  await genre.save();
  // insert the genre into the genre array
  genres[index] = genre;
...
async function bookCreate(index, title, summary, isbn, author, genre) {
  const bookdetail = {
    title: title,
    summary: summary,
    // here mongoose itself searches for the _id and assigns it as we have already defined
    // author: { type: Schema.Types.ObjectId, ref: "Author", required: true },
    author: author,
    isbn: isbn,
  };
  // same for genre as for author ref
...
async function createGenres() {
  ...
  // populates the genres array
  await Promise.all([
    // we pass in the arr_index and the name of the genre
...
async function createAuthors() {
  ...
  // now create authors
  await Promise.all([
    // we pass in the arr_index, first_name, family_name, dob, dod
